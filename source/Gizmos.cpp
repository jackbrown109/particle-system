...
void Gizmos::addBillboard(glm::vec4& a_position, glm::vec4& a_colour, glm::vec2 a_size, glm::mat4& a_camera)
{
	...
	glm::vec4 nvx = glm::vec4(X, 0.f);
	nvx = a_camera * nvx;

	// Vertex for the square
	vertex[0] = a_position.xyz - nvx.xyz - Y;
	vertex[1] = a_position.xyz - nvx.xyz + Y;
	vertex[2] = a_position.xyz + nvx.xyz + Y;
	vertex[3] = a_position.xyz + nvx.xyz - Y;

	// Creating the square with two tri's
	addTri(vertex[0], vertex[3], vertex[2], a_colour);
	addTri(vertex[0], vertex[2], vertex[1], a_colour);
}
